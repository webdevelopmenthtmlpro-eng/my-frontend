...
// Chinese (Simplified) - rough translations
...
// Arabic (right-to-left) - basic translations
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function sendMessageToBot() {
    ...
    const responses = [
      "I'll help you track your shipment right away.",
      "Let me check the delivery status for you.",
      "I can assist you with booking a delivery.",
      "Would you like to know more about our services?",
      "I'll connect you with our customer service team.",
      "Feel free to ask any questions about our shipping options."
    ];
    ...
    setTimeout(() => {
      ...
      botEl.textContent = responses[Math.floor(Math.random() * responses.length)];
  ...
  async function registerUser() {
    const username = $('#regUsername')?.value.trim(); const password = $('#regPassword')?.value.trim();
    if (!username || !password) { alert('Enter username and password'); return; }
    try {
      const res = await fetch('http://localhost:3000/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, password }) });
      const data = await res.json();
      if (res.ok) { alert('Registration successful'); $('#regUsername').value = ''; $('#regPassword').value = ''; } else { alert(data.message || 'Registration failed'); }
    } catch (err) { alert('Network error'); console.error(err); }
  }
  const registerBtn = $('#registerBtn');
  if (registerBtn) registerBtn.addEventListener('click', (e) => { e.preventDefault(); registerUser(); });

  // Contrast toggle (single)
  if (!$('#contrast-toggle-btn')) {
    const contrastBtn = document.createElement('button');
    contrastBtn.id = 'contrast-toggle-btn';
    contrastBtn.className = 'btn';
    contrastBtn.textContent = 'Ultra Contrast';
    contrastBtn.style.position = 'fixed';
    contrastBtn.style.bottom = '96px';
    contrastBtn.style.right = '120px';
    contrastBtn.style.zIndex = '2100';
    document.body.appendChild(contrastBtn);
    contrastBtn.addEventListener('click', () => {
      document.body.classList.toggle('dark-mode');
      const active = document.body.classList.toggle('high-contrast');
      contrastBtn.textContent = active ? 'Normal View' : 'Ultra Contrast';
    });
  }
